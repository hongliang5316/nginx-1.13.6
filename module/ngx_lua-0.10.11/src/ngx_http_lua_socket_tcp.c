...
ngx_http_lua_socket_tcp_gettimeouts(lua_State *L)
{
    ...
    if (u) {
        if (u->connect_timeout) {
            lua_pushinteger(L, u->connect_timeout);

        } else {
            lua_pushinteger(L, u->conf->connect_timeout);
        }

        if (u->send_timeout) {
            lua_pushinteger(L, u->send_timeout);

        } else {
            lua_pushinteger(L, u->conf->send_timeout);
        }

        if (u->read_timeout) {
            lua_pushinteger(L, u->read_timeout);

        } else {
            lua_pushinteger(L, u->conf->read_timeout);
        }
    }
